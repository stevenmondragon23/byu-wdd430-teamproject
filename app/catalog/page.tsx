...
export default async function CatalogPage({
  ...
  let filteredProducts = (products || []).filter((product: any) => {
    ...
    const sellerName = product.sellers?.name?.toLowerCase() || '';
  ...
  return (
    <div className="container" style={{ marginTop: '30px' }}>
      <h1 className={poppins.className} style={{ fontSize: '2.5rem', marginBottom: '20px' }}>
          ...
          filteredProducts.map((product: any) => (
            <div key={product.id} style={{ 
              ...
              <div style={{ height: '200px', backgroundColor: 'var(--secondary-color)' }}>
                {product.current_image_url && (
                  <img src={product.current_image_url} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                )}
              </div>
              
              <div style={{ padding: '20px' }}>
                <h3 className={poppins.className} style={{ margin: '0 0 10px 0' }}>{product.name}</h3>
                <Link href={`/seller/${product.sellers?.id}`} style={{ color: 'var(--accent-color)', textDecoration: 'none', fontWeight: 'bold' }}>
                  By: {product.sellers?.name || 'Vendedor'}
                </Link>
